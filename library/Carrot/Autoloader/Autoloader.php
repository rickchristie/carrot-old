 ...
 * </code>
 *
 * Autoloader will then look for class files in Carrot\Core
 * namespace inside that folder according to PSR-0 specs.
 *
...
{
    /**
     * Contains namespace to directory bindings. The path to
     * directory saved with a trailing directory separator. The
     * namespace saved without backslash prefix, but with a backslash
     * suffix.
     * 
     * @var array $namespaceBindings
     */
    private $namespaceBindings = array();
    
    /**
     * Contains class to file bindings, class name saved without
     * backslash prefix.
     * 
     * @var array $classBindings
     */
    private $classBindings = array();
    
    /**
     * Will be TRUE if this class has already registered its method
     ...
     * Constructor.
     * 
     * Automatically creates an instance of AutoloaderLog in
     * construction if none is provided.
     * 
     * @param AutoloaderLog $log
     *
     */
    public function __construct(AutoloaderLog $log = NULL)
    {
        if ($log == NULL)
        {
            $log = new AutoloaderLog;
        }
     ...
     * @param string $absoluteFilePath Absolute file path to the
     *        class file.
     *
     */
    ...
    /**
     * Registers loadClass() method using spl_autoload_register().
     *
     * @see loadClass()
     *
     */
    ...
    /**
     * Unregisters loadClass() method using spl_autoload_unregister().
     *
     * @see loadClass()
     *
     */
    ...
    /**
     * Get the AutoloaderLog instance of this autoloader.
     * 
     * @return AutoloaderLog
     *
     */
     ...
     * @return string Path to the directory with trailing directory
     *         separator.
     *
     */
     ...
     *
     * @throws RuntimeException If a class to file binding is found
     *         but the class file doesn't exist.
     * @param string $className Fully qualified class name, without
     *        backslash prefix.
     * @return bool TRUE if class file found and loaded.
     *
     */
    private function loadFromClassBindings($className)
    {
        if (array_key_exists($className, $this->classBindings))
        {
            ...
            $this->log->log($className, $this->classBindings[$className]);
            return TRUE;
        }
     ...
     *
     */
    private function loadFromNamespaceBindings($className)
    {
        foreach ($this->namespaceBindings as $binding)
        {
            ...
            {                
                ...
                {
                    require $filePath;
                    $this->log->log($className, $filePath);
                    return TRUE;
                }
            }
        }
     ...
     * PSR-0 specification.
     *
     * @param string $className Class name to convert (with
     *        namespace), without backslash prefix and suffix.
     * @param string $rootDirectory The root directory, with trailing
     *        directory separator.
     * @return string Path to the class file.
     *
     */
    private function convertToFilePath($className, $rootDirectory)
    {
     ...
     * </code>
     *
     * @param string $className Class name with namespace (without
     *        backslash prefix).
